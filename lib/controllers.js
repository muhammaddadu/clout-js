...
module.exports = function controllers(clout) {
	...
	// load modules API
	if (clout.modulesDir) {
		debug('Loading controllers from modules');
		utils.getGlobbedFiles(path.join(clout.modulesDir, '/*/controllers/**/*.js')).forEach(load);
	}

	var CONTROLLERS_DIR = path.join(clout.appDir, 'controllers');
	debug('CONTROLLERS_DIR: %s', CONTROLLERS_DIR);
	utils.getGlobbedFiles(CONTROLLERS_DIR + '/**/*.js').forEach(load);
