...
module.exports = function middleware(clout) {
	...
	if (clout.appDir) {
		...
		// use static files & views from modules
		if (clout.modulesDir) {
			debug('loading static folders & views from modules');
			var modules = fs.readdirSync(clout.modulesDir);
			for (var i = 0, l = modules.length; i < l; ++i) {
				var moduleName = modules[i],
					MODULE_PUBLIC_FOLDER = path.join(clout.modulesDir, moduleName, 'public'),
					MODULE_VIEWS_FOLDER = path.join(clout.modulesDir, moduleName, 'views');
				debug('Loading module: %s', moduleName);
				debug('MODULE_PUBLIC_FOLDER: %s', MODULE_PUBLIC_FOLDER);
				debug('MODULE_VIEWS_FOLDER: %s', MODULE_VIEWS_FOLDER);
				fs.existsSync(MODULE_PUBLIC_FOLDER) && clout.app.use(express.static(MODULE_PUBLIC_FOLDER));
				fs.existsSync(MODULE_VIEWS_FOLDER) && modules.push(MODULE_VIEWS_FOLDER);
			}
		}

		views.push(CLOUT_VIEWS_DIR); // load clout views last
		clout.app.use(express.static(PUBLIC_DIR)); // public folder
		clout.app.set('views', views); // set views folder
	...
	clout.app.use(function(req, res, next) {
		...
		function createJsonResponse(type, opts) {
			return function(payload) {
				...
				// Add support for parsing sequalize errors
