...
function Clout(rootDirectory) {
	...
	if (this.applicationPackage.modules) {
		...
		this.applicationPackage.modules.forEach(function (moduleName) {
	...
	// 2) load application configuration (Overrides module conf)
	...
	this.loadHooksFromDir(path.join(__dirname, '..'))
		...
		.then(function () {
			// load module hooks
			async.forEach(self.modules, function (module, next) {
 ...
 * @param {String} 		fn.name    	hook name
...
Clout.prototype.registerHook = function registerHook(event, fn, priority, override) {
	...
	if (override === true) {	
		for (var i = 0, l = this.hooks[event]; i < l; ++i) {
			var hook = this.hooks[event][i];
			if (hook.name === fn.name && hook.group === fn.group) {
	...
	for (var i = 0, l = this.hooks[event]; i < l; ++i) {
		...
		if (fn.priority < tmp_p) {
			...
			return this.hooks[event].insert(i, fn);
		}
	}
